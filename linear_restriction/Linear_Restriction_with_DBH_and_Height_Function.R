#---------------------------------------------------------------#
# Intercept Estimation with linear Restriction for 2 Regressors #
#---------------------------------------------------------------#

rlm_dh <- function(komp, BHD, Height, Vollbaum, n_boot = 10000){
  # Eingangsvariablen:
...
Parameter <- matrix(0, nrow = n_boot, ncol = 12)
...
for(i in 1 : n_boot){
  ...
  a.3 <- matrix(Height[c(Z)], nrow = 3, ncol = 4)
  A <- cbind(a.1, a.2, a.3)
  ...
  if(!any(is.na(Beta_R))){
    Parameter[i,] <- Beta_R # TODO : Fehler sauber abfangen!!
  }
...
g3 <- median(Parameter[,11])
g4 <- median(Parameter[,12])

# Rueckgabe
param_return <- c(a1, a2, a3, a4, b1, b2, b3, b4, g1, g2, g3, g4)
